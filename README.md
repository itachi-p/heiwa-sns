...
ニックネーム列・検証・一覧表示用の `users` 参照権限は、マイグレーション  
`supabase/migrations/20260324120000_users_nickname.sql`  
を適用してください（`users.nickname` は `text`、未設定は `null`。DB でも 1〜20 文字・改行不可を CHECK で制約）。
