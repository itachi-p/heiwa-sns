...
- **公開ID未設定**（`PublicIdRequiredLayer`）: ログイン済み・招待オンボーディング完了・パスワード変更不要のとき、`users.public_id` が **null・空**ならブロッキングで入力（`components/public-id-required-layer.tsx`・`POST /api/set-public-id`）。入力規則は `lib/public-id.ts`（5〜20文字・英数字と `._-`）。表示は `/@{public_id}`（middleware で `/p/{handle}` にリライト）。
