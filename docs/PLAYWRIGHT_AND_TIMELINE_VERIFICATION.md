...
**Supabase / ログインが必要なシナリオ**を E2E に書く場合は、`.env.local` の有無・テスト用アカウント・CI のシークレットなどを別途設計する。ファイル別の一覧・注意は **`e2e/PLAYWRIGHT-INDEX.md`**。**`e2e/timeline-toxicity-filter.spec.ts`** は閲覧フィルタの帯を確定させるため、テスト用投稿の `moderation_max_score` を **`SUPABASE_SERVICE_ROLE_KEY` で上書き**する（`NEXT_PUBLIC_SUPABASE_URL` も必要）。API の自動採点だけでは帯が揺れるため。**`e2e/unit-timeline-sort.spec.ts`** はブラウザを使わず `lib/timeline-sort.ts` のみを検証する。
